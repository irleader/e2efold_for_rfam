...
from e2efold_rfam.common.long_seq_pre_post_process import *
...
class RNASSDataGenerator(object):
    ...
    def load_data(self):
        ...
        self.conservation=np.array([instance[4] for instance in self.data])

        self.pairs = np.array([instance[-1] for instance in self.data],dtype=object) #########warning
    ...
    def upsampling_data(self):
        name = [instance[3] for instance in self.data]
        d_type = np.array(list(map(lambda x: x.split('/')[0], name)))
    ...
    def subs2map(self, conservation):
        seq_len = self.seq_max_len
        subs = np.zeros([seq_len, seq_len])
        for c in conservation:
            if c[-1]=='0':
                subs[int(c[0])-1,int(c[1])-1]=10
                subs[int(c[1])-1,int(c[0])-1]=10
            elif c[-2]=='doublesub':
                subs[int(c[0])-1,int(c[1])-1]=10
                subs[int(c[1])-1,int(c[0])-1]=10
            else:
                if subs[int(c[0])-1,int(c[1])-1]==0 or subs[int(c[1])-1,int(c[0])-1]==0:
                    subs[int(c[0])-1,int(c[1])-1]=5
                    subs[int(c[1])-1,int(c[0])-1]=5
    ...
    def next_batch_SL(self, batch_size):
        ...
        if self.batch_pointer >= len(self.data_x):
            self.batch_pointer = 0
        #p = Pool()
        #contact = np.array(list(p.map(self.pairs2map, data_pairs)))
    ...
    def get_one_sample(self, index):
        ...
        conservation = self.conservation[index]

        contact= self.pairs2map(data_pair)
        matrix_rep = np.zeros(contact.shape)
        return contact, data_seq, matrix_rep, data_len, conservation # pair,seq,zeros,length
