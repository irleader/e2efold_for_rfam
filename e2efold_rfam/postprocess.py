...
from e2efold_rfam.common.utils import unravel2d_torch, F1_low_tri, encoding2seq
from e2efold_rfam.common.utils import constraint_matrix, constraint_matrix_batch, constraint_matrix_batch_diag
...
# nonlinear transformation Tau(a_hat)
...
def postprocess(u, x, lr_min, lr_max, num_itr, rho=0.0, with_l1=False):
    """
    :param u: utility matrix, u is assumed to be symmetric, in batch :NLL
    :param x: RNA sequence, in batch :NL4
    ...
    #m=constraint_matrix_batch_diag(x) #makes no difference
    ...
    #U ← softsign(U − s) ◦ U
    u = soft_sign(u - math.log(9.0)) * u #NLL

    # initialization
    #A0← softsign(U − s) ◦ sigmoid(U)
    a_hat = (torch.sigmoid(u)) * soft_sign(u - math.log(9.0)).detach() #NLL
    #λ0← w · relu(A01 − 1)
    lmbd = F.relu(torch.sum(contact_a(a_hat, m), dim=-1) - 1).detach() #NLL --> N*L

    # gradient descent
    for t in range(num_itr):

        grad_a = (lmbd * soft_sign(torch.sum(contact_a(a_hat, m), dim=-1) - 1)).unsqueeze_(-1).expand(u.shape) - u / 2 #NLL
        grad = a_hat * m * (grad_a + torch.transpose(grad_a, -1, -2)) #NLL
        a_hat -= lr_min * grad #NLL
        lr_min = lr_min * 0.99

        if with_l1:
            a_hat = F.relu(torch.abs(a_hat) - rho * lr_min)

        lmbd_grad = F.relu(torch.sum(contact_a(a_hat, m), dim=-1) - 1) #NL
        lmbd += lr_max * lmbd_grad #NL
        lr_max = lr_max * 0.99 #NL
