...
import math
from e2efold_rfam.common.utils import soft_sign
import numpy as np
...
class ContactAttention_simple(nn.Module):
    ...
    def matrix_rep(self, x):
        ...
        x2 = x2.unsqueeze(2) # N*L*1*d
        x = x.repeat(1, L,1,1) #N*L*L*d
        x2 = x2.repeat(1, 1, L,1)#N*L*L*d
        mat = torch.cat([x,x2],-1) # N*L*L*2d

        # make it symmetric
        # mat_tril = torch.cat(
        #     [torch.tril(mat[:,:, i]) for i in range(mat.shape[-1])], -1)
        mat_tril = torch.tril(mat.permute(0, -1, 1, 2)) # N*2d*L*L -->N*2d*L*L
        mat_diag = mat_tril - torch.tril(mat.permute(0, -1, 1, 2), diagonal=-1)# N*2d*L*L
        mat = mat_tril + torch.transpose(mat_tril, -2, -1) - mat_diag # N*2d*L*L
        return mat
...
class ContactAttention_simple_fix_PE(ContactAttention_simple):
    ...
    def forward(self, pe, seq, state):
        ...
        seq = seq.permute(0, 2, 1) # N*4*L
        seq = F.relu(self.bn1(self.conv1d1(seq))) #N*d*L just for increase the capacity

        seq = torch.cat([seq, position_embeds], 1) # N*2d*L
        seq = self.transformer_encoder(seq.permute(-1, 0, 1)) # L*N*2d -->L*N*2d
        seq = seq.permute(1, 2, 0) #N*2d*L

        # what about apply attention on the the 2d map?
        seq_mat = self.matrix_rep(seq) # N*4d*L*L

        p_mat = self.matrix_rep(position_embeds) # N*2d*L*L

        infor = torch.cat([seq_mat, p_mat], 1) # N*6d*L*L

        contact = F.relu(self.bn_conv_1(self.conv_test_1(infor))) #N*d*L*L
        contact = F.relu(self.bn_conv_2(self.conv_test_2(contact))) #N*d*L*L
        contact = self.conv_test_3(contact) #N*1*L*L

        contact = contact.view(-1, self.L, self.L) #N*L*L
        contact = (contact+torch.transpose(contact, -1, -2))/2 #N*L*L

        return contact.view(-1, self.L, self.L) #N*L*L
...
class Lag_PP_mixed_conserved(Lag_PP_zero):
    """
    ...
    def __init__(self, steps, k, rho_mode='fix'):
        super(Lag_PP_mixed_conserved, self).__init__(steps, k)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.steps = steps # ﬁxed # iterations，PPcellφ is applied recursively for T iterations
        self.k = k #softsign(c) := 1/(1 + exp(−kc)), where k is a temperature.

        # self.s = nn.Parameter(torch.ones(600, 600)*math.log(9.0))
        # self.s = math.log(9.0)
        self.s = nn.Parameter(torch.Tensor([math.log(9.0)])) #offset term
        self.w = nn.Parameter(torch.randn(1))
        self.rho = nn.Parameter(torch.Tensor([1.0]))
        # self.rho = 1.0

        self.rho_m = nn.Parameter(torch.randn(600, 600)) # used for rho_mode='matrix'
    ...
    def forward(self, u, x, conservation_batch):
        a_t_list = list()
        a_hat_t_list = list()
        lmbd_t_list = list()

        m=self.constraint_matrix_batch(x)
        m1=self.constraint_matrix_conserved_batch(x,conservation_batch)#.mul(u) # N*L*L
        ...
        a_hat_tmp = (torch.sigmoid(u)) * soft_sign(u - self.s, self.k).detach()
        #A0← T (Aˆ0)
        a_tmp = self.contact_a(a_hat_tmp, m1)  #####1
        #λ0← w · relu(A01 − 1)
        ...
        # gradient descent
        for t in range(self.steps):
            lmbd_updated, a_updated, a_hat_updated = self.update_rule(u, m, lmbd_tmp, a_tmp, a_hat_tmp, t,m1)
    ...
    def update_rule(self, u, m, lmbd, a, a_hat, t,m1):
...
class RNA_SS_e2e_conserved(nn.Module):
    def __init__(self, model_att, model_pp):
        super(RNA_SS_e2e_conserved, self).__init__()
        self.model_att = model_att
        self.model_pp = model_pp

    def forward(self, prior, seq, state, conservation_batch):
        u = self.model_att(prior, seq, state)
...
class ContactNetwork(nn.Module):
    ...
    def __init__(self, d, L):
        ...
        # if possible, we may think of make dilation related to the sequence length
